...
fetch("https://kanap-backend.lauravongsy.repl.co/api/products")
    .then((res) => {
        return res.json();
    })
    .then(function (data) {
...
function changeQuantity() {
    ...
    for (let i = 0; i < changeQty.length; i++) {
        changeQty[i].addEventListener("change", (event) => {
            ...
            const findModif = panier.find(
                (e) => e.modifValue !== modifQuantity
            );
    ...
    for (let i = 0; i < btnDelete.length; i++) {
        btnDelete[i].addEventListener("click", (event) => {
            ...
            panier = panier.filter(
                (e) =>
                    e.articleId !== idDelete || e.articleColor !== colorDelete
            );
...
function orderCart() {
    ...
    btnOrder.addEventListener("click", (e) => {
        ...
        if (!panier) {
            ...
            fetch(
                "https://kanap-backend.lauravongsy.repl.co/api/products/order",
                postOptions
            )
                .then((res) => {
                    return res.json();
